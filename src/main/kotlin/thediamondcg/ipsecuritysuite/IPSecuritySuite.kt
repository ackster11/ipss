...
//            commandDispatcher.register(setApiKey)
        ...
        ServerPlayConnectionEvents.INIT.register { serverPlayNetworkHandler: ServerPlayNetworkHandler,
            ...
            try {
                ...
                println("IPSS: Login IP ${player.ip} is " + if(ipData.strictlyValidIp(player.uuid, playerIp)) "approved for logon" else "unregistered")
                ...
                if (!validIp) {
                    serverPlayNetworkHandler.disconnect(Text.literal("Someone tried logging into account ${player.name} with unauthorized IP: $playerIp"))
